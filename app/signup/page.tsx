...
export default function SignUpPage() {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    // Simulate API registration call (Better Auth flow)
    setTimeout(() => {
      setIsLoading(false);
      setIsSuccess(true);
      setTimeout(() => {
        // Redirect to owner dashboard or signin on success
        router.push("/signin");
      }, 1500);
    }, 1000);
  };
    ...
    <div className="min-h-dvh flex flex-col bg-background text-text-primary">
      ...
      <main className="flex-1 flex items-center justify-center p-6 sm:p-10">
        <div className="w-full max-w-md space-y-6">
          ...
          {/* Success Banner */}
          {isSuccess && (
            <div className="p-4 rounded-xl border border-success/30 bg-success/10 text-success text-sm font-medium flex items-center gap-2">
              <span className="text-base">✓</span>
              <span>Business created successfully! Redirecting to sign in...</span>
            </div>
          )}
