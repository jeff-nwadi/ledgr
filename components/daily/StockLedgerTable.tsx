...
export function StockLedgerTable({ ledger: initialLedger }: StockLedgerTableProps) {
  ...
  return (
    <div className="bg-background border border-border/50 rounded-[1.25rem] overflow-hidden">
        ...
        <table className="w-full text-left text-[13px]">
          <thead className="bg-surface/50 border-b border-border/40 text-text-muted">
            <tr>
              ...
              <th className="px-6 py-4 text-right text-success">Sold</th>
          ...
          <tbody className="divide-y divide-border/30">
            {ledger.map((row) => (
              <tr key={row.productId} className="hover:bg-surface/30 transition-colors">
                ...
                <td className="px-6 py-4 text-text-muted text-right">{row.openingQty}</td>
                <td className="px-6 py-4 text-text-muted text-right">{row.addedQty}</td>
                <td className="px-6 py-4 text-success text-right">{row.soldQty}</td>
                <td className="px-6 py-4 text-danger text-right">{row.wasteQty}</td>
                <td className="px-6 py-4 text-brand text-right bg-brand/5">{row.calculatedClosingQty}</td>
                
                <td className="px-6 py-4 text-right">
                  {row.varianceQty !== undefined && row.varianceQty !== null ? (
                    <span className="text-text-primary">{row.countedClosingQty}</span>
                  ) : (
                    <input 
                      type="number"
                      min="0"
                ...
                <td className="px-6 py-4 text-right">
                  {row.varianceQty === undefined || row.varianceQty === null ? (
                    <button
                      ...
                      className="inline-flex items-center gap-1.5 px-3 py-1.5 bg-background border border-border/50 hover:bg-surface text-text-primary text-xs rounded-full transition-colors disabled:opacity-50"
                    >
