...
export function DailyStockTable({ entries }: { entries: any[] }) {
  ...
  return (
    <div className="rounded-[1rem] bg-background border border-border/50 overflow-hidden">
      <div className="p-4 sm:p-5 border-b border-border/50 bg-surface/30">
        <h2 className="text-sm sm:text-[15px] text-text-primary">Stock Ledger</h2>
          ...
          return (
            ...
            >
              ...
              <div className="flex items-start justify-between gap-2">
                <div>
                  <h3 className="text-text-primary text-base leading-snug">{entry.productName}</h3>
                  <span className="text-xs text-text-muted">{entry.productUnit || "Unit"}</span>
                </div>
                {isUncounted ? (
                  <span className="px-2.5 py-1 rounded-full text-xs bg-surface text-text-muted border border-border/60">
                    Not Counted
                  </span>
                ) : entry.varianceQty === 0 ? (
                  <span className="inline-flex items-center gap-1 px-2.5 py-1 rounded-full text-xs bg-emerald-500/10 text-emerald-600 border border-emerald-500/20">
                    <CheckCircle2 className="w-3.5 h-3.5" /> Balanced
                  </span>
                ) : (
                  <span className="inline-flex items-center gap-1 px-2.5 py-1 rounded-full text-xs bg-rose-500/10 text-rose-600 border border-rose-500/20">
              ...
              <div className="bg-background/80 p-2.5 rounded-lg border border-border/30 overflow-x-auto">
                ...
                <div className="flex items-center gap-1.5 text-xs whitespace-nowrap">
                  <div className="px-2 py-1 bg-surface rounded-md text-text-primary">
                    Open: <span className="">{entry.openingQty}</span>
                  </div>
                  <ArrowRight className="w-3 h-3 text-text-muted/60 flex-shrink-0" />
                  <div className="px-2 py-1 bg-surface rounded-md text-text-primary">
                    Added: <span className="">{entry.addedQty > 0 ? `+${entry.addedQty}` : "0"}</span>
                  </div>
                  <ArrowRight className="w-3 h-3 text-text-muted/60 flex-shrink-0" />
                  <div className="px-2 py-1 bg-surface rounded-md text-text-primary">
                    Sold: <span className="">{entry.soldQty > 0 ? `-${entry.soldQty}` : "0"}</span>
                  </div>
                  <ArrowRight className="w-3 h-3 text-text-muted/60 flex-shrink-0" />
                  <div className="px-2 py-1 bg-surface rounded-md text-text-primary">
                    Waste: <span className="">{entry.wasteQty > 0 ? `-${entry.wasteQty}` : "0"}</span>
                  </div>
                </div>
              </div>
              ...
              <div className="grid grid-cols-3 gap-2 pt-1 text-center text-xs">
                <div className="p-2 rounded-lg bg-surface/60">
                  ...
                  <span className="text-text-primary">{entry.calculatedClosingQty}</span>
                </div>
                <div className="p-2 rounded-lg bg-surface/60">
